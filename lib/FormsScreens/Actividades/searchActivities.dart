import '../../config.dart';
import './registerAssistance.dart';
...
class _SearchActivitiesScreenState extends State<SearchActivitiesScreen> {
  ...
  Future<void> searchUser() async {
    ...
    if (response.statusCode == 200) {
      ...
      Navigator.push(
        ...
        MaterialPageRoute(
            builder: (context) =>
                RegistrarAsistenciaForm(UserID: username, PersonalID: PersonalID)),
      );
    } else {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('Buscar Usuario Act $personalID'),
      ),
      body: Padding(
...
void main() {
  ...
  runApp(MaterialApp(
    home: SearchActivitiesScreen(personalID: personalID),
  ));
}
